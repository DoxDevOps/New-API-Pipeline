...
for site_id in cluster['site']:
    ...
    while (count < 1):
        ...
        if subprocess.call(['ping', param, '1', site['ip_address']]) == 0:
            ...
            ship_api = "rsync " + "-r $WORKSPACE/BHT-EMR-API " + site['username'] + "@" + site['ip_address'] + ":/var/www"
            os.system(ship_api)

            # ship data to remote site
            #api_add = "cd /$WORKSPACE/BHT-EMR-API " + "git remote add " + site['username'] + "@" + site[
                #'ip_address'] + ":/var/www/BHT-EMR-API"
            #os.system(api_add)
            
             # ship data to remote site
            #push_api = "cd /$WORKSPACE/BHT-EMR-API " + "git push origin master"
            #os.system(push_api)
